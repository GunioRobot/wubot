...
Wubot::Guide::Tasks - guide to using wubot to monitor tasks
...
L<Wubot::Plugin::EmacsOrgMode> - monitors ~/org directory for new
files that contain TODO items that have a scheduled or deadline date.

L<Wubot::Plugin::TaskDB> - monitors the task database for the highest
priority task in the queue.

L<Wubot::Plugin::TaskNotify> - monitors the task database for items
that have a scheduled or deadline date that is due within the next 15
minutes.

L<Wubot::Util::Tasks> - utility class where most of the parsing and
database work is done.  Used by the other classes.

L<Wubot::Web::Tasks> - web user interface for tasks.
