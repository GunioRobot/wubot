...
# active duration
{
    ...
    is( ( $idle->calculate_idle_stats( $now, 0, {}, {} ) )[0]->{active_min},
        0,
        "Checking that calculate_idle_status calculated initial active_state_min as 0"
    );

    is( ( $idle->calculate_idle_stats( $now, 0, {}, {} ) )[0]->{idle_state},
        0,
        "Checking that calculate_idle_status set idle_state false with no idle time"
    );

    is( ( $idle->calculate_idle_stats( $now, 600, {}, {} ) )[0]->{idle_state},
        1,
        "Checking that calculate_idle_status set idle_state after 10 minutes"
    );

    is( ( $idle->calculate_idle_stats( $now, 600, {}, {} ) )[0]->{lastupdate},
        $now,
        "Checking that calculate_idle_status set lastupdate to current time"
    );

    is( ( $idle->calculate_idle_stats( $now, 0, {}, { lastupdate => $now-601, active_since => $now-601 } ) )[0]->{active_since},
        $now,
        "Checking that calculate_idle_status reset active_since when data is expired"
    );

    is( ( $idle->calculate_idle_stats( $now, 0, {}, { lastupdate => $now-601, active_since => $now-601 } ) )[0]->{cache_expired},
        1,
        "Checking that calculate_idle_status set cache_expired flag when cache is too old"
    );

    is( ( $idle->calculate_idle_stats( $now, 0, {}, { lastupdate => $now-60, cache_expired => 1 } ) )[0]->{cache_expired},
        undef,
    ...
    is( ( $idle->calculate_idle_stats( $now, 600, {}, {} ) )[0]->{idle_min},
        '10',
        "Checking that calculate_idle_status calculated 10 minutes of idle status"
    );

    is( ( $idle->calculate_idle_stats( $now, 600, {}, { idle_since => $now-1200 } ) )[0]->{idle_min},
        10,
...
# active after idle
{
    ...
    is( ( $idle->calculate_idle_stats( $now, 0, {}, $cache ) )[0]->{idle_state},
        0,
        "Checking for idle_state flag when going active"
    );

    is( ( $idle->calculate_idle_stats( $now, 0, {}, $cache ) )[0]->{idle_state_change},
        1,
...
# idle after active
{
    ...
    is( ( $idle->calculate_idle_stats( $now, 60*15, {}, $cache ) )[0]->{idle_state},
        1,
        "Checking for idle_state flag when going active"
    );

    is( ( $idle->calculate_idle_stats( $now, 60*15, {}, $cache ) )[0]->{idle_state_change},
        1,
        "Checking for idle_state_change flag when going active"
    );
