...
rule tree and extecutes any rules whose conditions evealuate to true
for the message.
...
classes that are referenced.  This makes it possible to initalize each
plugin without having to wait for a rule that references the plugin to
fire.
