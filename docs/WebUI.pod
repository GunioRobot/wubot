...
Wubot::Guide::WebUI - guide for using the wubot web interface
...
The web ui can be used to serve out RSS feeds.  You can take data from
one or more incoming feeds (from wubot-monitor), filter it, modify it
(e.g. strip html or images), and so forth.  You can combine multiple
feeds together into a single RSS feed or you can take a single RSS
feed and split it out into multiple feeds.  You can even take data
from other sources (e.g. an mbox, irc), use some rules to adapt the
schema, and then send it out as an RSS feed.
...
L<Wubot::Guide::GettingStarted> document.
