...
the RSS monitor, see L<Wubot::Plugin::RSS>.
...
properties of a rule, have a peak at L<Wubot::Guide::Rules>.  For an
overview of the available reactor plugins, see
L<Wubot::Guide::ReactorPlugins>.
...
To enable the notifications, see L<Wubot::Guide::Notifications>.
...
notifications rules discussed in L<Wubot::Guide::Notifications>.
...
To start the webui process, copy the 'webui' directory out of the
distribution tarball, set into the 'webui' directory, and run the
script ./wubot-webui.  Once the process is started up, point your RSS
reader here, and enjoy your filtered feed:
